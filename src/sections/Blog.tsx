      ...
      <div className="max-w-[1100px] mx-auto px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {blogConfig.posts.map((post, index) => (
            <article
              key={post.id}
              className={`group relative h-[450px] overflow-hidden cursor-pointer transition-all duration-700 rounded-xl ${
            ...
            >
              ...
              <div
                className="absolute inset-0 bg-gradient-to-t from-cls-dark/90 via-cls-dark/50 to-transparent opacity-60 group-hover:opacity-90 transition-opacity duration-600"
              />

              {/* Content */}
              <div className="absolute inset-0 flex flex-col justify-end p-6 text-white">
                {/* Date */}
                <span className="text-sm font-medium tracking-wide opacity-80 mb-3 transform translate-y-0 group-hover:-translate-y-4 transition-transform duration-600">
                  <i className="fa-regular fa-calendar mr-2"></i>
                  {post.date}
                </span>

                {/* Title */}
                <h3 className="font-sans text-xl md:text-2xl leading-tight mb-4 transform translate-y-0 group-hover:-translate-y-4 transition-transform duration-600 font-semibold">
                  {post.title}
                </h3>
                ...
                <div className="flex items-center gap-2 opacity-0 transform translate-y-4 group-hover:opacity-100 group-hover:translate-y-0 transition-all duration-600">
                  <span className="text-sm tracking-[0.2em] uppercase font-medium">{blogConfig.readMoreText}</span>
                  <i className="fa-solid fa-arrow-right"></i>
                  {/* Underline animation */}
                  <div className="absolute bottom-6 left-6 h-[2px] bg-cls-lime w-0 group-hover:w-24 transition-all duration-600" />
                </div>
              </div>
            </article>
          ))}
