    ...
    <section id="about" className="relative">
      {aboutConfig.sections.map((section, index) => (
        <AboutSection
          ...
          reverse={index % 2 !== 0}
        >
          <span className="inline-block mb-4 text-sm tracking-[0.2em] font-semibold uppercase opacity-70">
            {section.tag}
          </span>
          <h2 className="font-sans text-3xl md:text-4xl leading-tight mb-6 font-bold">
            {section.heading}
          </h2>
          {section.quote ? (
            <>
              <p className="text-lg font-medium leading-relaxed opacity-90 mb-6 italic">
            ...
            section.paragraphs.map((paragraph, pIndex) => (
              <p key={pIndex} className="text-lg font-normal leading-relaxed opacity-90 mb-6">
                {paragraph}
              </p>
            ))
          )}
